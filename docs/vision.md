...
- **Embeddings are pluggable.** PostgreSQL (vectordb) supports swapping embedding models. As domain-specific geospatial embedding models emerge, they can be substituted without changing the pipeline.
...
- **No lock-in.** The output is standard JSON and a local PostgreSQL (vectordb) directory. The tool does not require any proprietary platform.
